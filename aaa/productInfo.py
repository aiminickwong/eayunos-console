...
class Product(object):
   ...
   def __init__(self, loop):
      ...
      # 如果该用户是有权限的用户，就显示该用户的注册机构

      self.username = ''
      self.password = ''
      self.key = ''

      if self.get_username() == 'admin' and self.get_password() == 'admin': 
         # self.info 返回值类型是个列表
         ...
         # 新的组件：显示的是产品信息
         self.txtInfo = urwid.Text(info_txt, align = 'center')
   ...
   # 获取到 login 中的用户名，密码和 key，然后设置到该 product 类中
